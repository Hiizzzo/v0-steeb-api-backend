    ...
    await db.runTransaction(async (transaction) => {
      ...
      if (oldTipo !== tipoUsuario) {
        ...
        if (statsDoc.exists) {
          ...
          else if (oldTipo === 'dark' || oldTipo === 'black') updates["3_darkUsers"] = admin.firestore.FieldValue.increment(-1);
          else updates["2_whiteUsers"] = admin.firestore.FieldValue.increment(-1);

          // Incrementar nuevo
          if (tipoUsuario === 'shiny') updates["4_shinyUsers"] = admin.firestore.FieldValue.increment(1);
          else if (tipoUsuario === 'dark' || tipoUsuario === 'black') updates["3_darkUsers"] = admin.firestore.FieldValue.increment(1);
          else updates["2_whiteUsers"] = admin.firestore.FieldValue.increment(1);
