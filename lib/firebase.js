...
export const updateUserTipo = async (userId, tipoUsuario, permissions = []) => {
  try {
    ...
    await db.runTransaction(async (transaction) => {
      // 1. Obtener usuario actual para saber su rol anterior
      const userDoc = await transaction.get(userRef);
      if (!userDoc.exists) {
        throw new Error('User does not exist!');
      }
      
      const oldTipo = userDoc.data().tipoUsuario || 'white';
      ...
      // 3. Actualizar estadísticas si el rol cambió
      if (oldTipo !== tipoUsuario) {
        ...
        if (statsDoc.exists) {
          ...
          if (oldTipo === 'shiny') updates.shinyUsers = admin.firestore.FieldValue.increment(-1);
          else if (oldTipo === 'dark' || oldTipo === 'black') updates.darkUsers = admin.firestore.FieldValue.increment(-1);
          else updates.whiteUsers = admin.firestore.FieldValue.increment(-1);

          // Incrementar nuevo
          if (tipoUsuario === 'shiny') updates.shinyUsers = admin.firestore.FieldValue.increment(1);
          else if (tipoUsuario === 'dark' || tipoUsuario === 'black') updates.darkUsers = admin.firestore.FieldValue.increment(1);
          else updates.whiteUsers = admin.firestore.FieldValue.increment(1);
