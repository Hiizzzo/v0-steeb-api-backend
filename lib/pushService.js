...
let pendingVapidResolution = null;

const getVapidKeysFromEnv = () => ({
  publicKey: VAPID_PUBLIC_KEY,
  privateKey: VAPID_PRIVATE_KEY
});

const fetchVapidKeysFromFirestore = async () => {
  try {
    const doc = await db.collection(PUSH_CONFIG_COLLECTION).doc(PUSH_CONFIG_DOC).get();
    if (!doc.exists) return null;
    const data = doc.data() || {};
    const publicKey = data.vapidPublicKey || data.publicKey || '';
    const privateKey = data.vapidPrivateKey || data.privateKey || '';
    ...
    console.warn('No se pudo leer la configuracion VAPID desde Firestore:', error.message || error);
    return null;
  }
};

const resolveVapidKeys = async () => {
  if (cachedVapidKeys?.publicKey && cachedVapidKeys?.privateKey) {
    return cachedVapidKeys;
  }

  if (!pendingVapidResolution) {
    pendingVapidResolution = (async () => {
      const envKeys = getVapidKeysFromEnv();
      if (envKeys.publicKey && envKeys.privateKey) {
        cachedVapidKeys = envKeys;
        return cachedVapidKeys;
      }

      const firestoreKeys = await fetchVapidKeysFromFirestore();
      if (firestoreKeys?.publicKey && firestoreKeys?.privateKey) {
        cachedVapidKeys = firestoreKeys;
        return cachedVapidKeys;
      }
      ...
      pendingVapidResolution = null;
    });
  }

  return pendingVapidResolution;
};

let webPushConfigured = false;

const ensureWebPushConfigured = async () => {
  if (webPushConfigured) return true;

  const keys = await resolveVapidKeys();
  if (keys.publicKey && keys.privateKey) {
    webpush.setVapidDetails(`mailto:${PUSH_SENDER_EMAIL}`, keys.publicKey, keys.privateKey);
  ...
  console.warn(
    'Web Push deshabilitado: faltan VAPID_PUBLIC_KEY o VAPID_PRIVATE_KEY. Configura las variables de entorno o el documento config/push en Firestore.'
  );
  return false;
};

export const getPublicPushConfig = async () => {
  const keys = await resolveVapidKeys();
...
const isPushConfigured = async () => ensureWebPushConfigured();
