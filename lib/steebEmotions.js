...
const EMOTION_STORAGE_KEY = 'steebEmotionImage';
...
export function getSteebEmotion(message = '', context = {}) {
  ...
  return {
    name,
    image: getSteebEmotionImagePath(name),
...
export function getSteebEmotionImagePath(emotion = 'happy') {
  return EMOTION_IMAGES[emotion] || EMOTION_IMAGES.happy;
}

export function setSteebEmotionImage(emotion = 'happy') {
  const imagePath = getSteebEmotionImagePath(emotion?.name || emotion);
  try {
    if (typeof localStorage !== 'undefined') {
      localStorage.setItem(EMOTION_STORAGE_KEY, imagePath);
    }
  } catch (error) {
...
export function getStoredSteebEmotionImage() {
  try {
    ...
    console.warn('No se pudo leer la emoción de Steeb en localStorage:', error);
  }
  return getSteebEmotionImagePath('happy');
}
